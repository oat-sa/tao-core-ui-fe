...
var getActualRatio = function getActualRatio(conf) {
    ...
    if (conf.sizeProps.px.current.width > 0 && conf.sizeProps.px.current.height > 0) {
        conf.sizeProps.ratio.current = gamp.round(
            conf.sizeProps.px.current.width / conf.sizeProps.px.current.height,
            conf.precision
        );
    }
...
var setPercent = function setPercent(conf, percent, maxWidth) {
    ...
    conf.sizeProps.px.current.height = gamp.round(
        conf.sizeProps.px.current.width / conf.sizeProps.ratio.natural,
        conf.precision
    );
    return conf;
};

...
define([
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.equal(
            typeof resourceSelectorFactory,
            'function',
            'The resourceSelectorFactory module exposes a function'
        );
        ...
        assert.notStrictEqual(
            ...
            resourceSelectorFactory(),
            'The resourceSelectorFactory provides a different object on each call'
        );
    });
    ...
    QUnit.test('multiple selection rendering', function(assert) {
        ...
        resourceSelectorFactory($container, {
            ...
            assert.equal(
                $('.context .class-selector.rendered', $element).length,
                1,
                'The component has the class selector'
            );
            ...
            assert.equal(
                $('.selection .selection-control input', $element).length,
                1,
                'The component has the selection control'
            );
            assert.ok(
                !$('.selection .selection-control label', $element).hasClass('hidden'),
                'The selection control is displayed'
            );
    ...
    QUnit.test('single selection rendering', function(assert) {
        ...
        resourceSelectorFactory($container, {
            ...
            assert.ok(
                $('.selection .selection-control label', $element).hasClass('hidden'),
                'The selection control is hidden'
            );
        ...
        resourceSelectorFactory($container, config)
            .on('update', function() {
                ...
                assert.equal(
                    ...
                    'undefined',
                    'The selection does not contain node1'
                );
                ...
                assert.equal(
                    ...
                    'undefined',
                    'The selection does not contain noder2'
                );
                ...
                assert.equal(
                    typeof selection['http://bertao/tao.rdf#i14918988538969120'],
                    'undefined',
                    'The selection does not contain node2'
                );
        ...
        resourceSelectorFactory($container, config)
            .on('update', function() {
                ...
                if (clickState === 1) {
                    ...
                    assert.equal(
                        typeof selection[$subnode1.data('uri')],
                        'undefined',
                        'The selection doesnt contain subnode1'
                    );

                    assert.ok(!$subnode2.hasClass('selected'), 'subnode2 is deselected');
                    assert.equal(
                        typeof selection[$subnode2.data('uri')],
                        'undefined',
                        'The selection doesnt contain subnode2'
                    );

                    assert.ok(!$subnode3.hasClass('selected'), 'subnode3 is deselected');
                    assert.equal(
                        typeof selection[$subnode3.data('uri')],
                        'undefined',
                        'The selection doesnt contain subnode3'
                    );
                    ...
                    assert.equal(
                        typeof selection[$node1.data('uri')],
                        'undefined',
                        'The selection doesnt contain node1'
                    );

                    assert.ok(!$node2.hasClass('selected'), 'node2 is deselected');
                    assert.equal(
                        typeof selection[$node2.data('uri')],
                        'undefined',
                        'The selection doesnt contain node2'
                    );

                    assert.ok(!$node3.hasClass('selected'), 'node3 is deselected');
                    assert.equal(
                        typeof selection[$node3.data('uri')],
                        'undefined',
                        'The selection doesnt contain node3'
                    );
            ...
            .on('update.foo', function() {
                ...
                this.on('query', function(params) {
                    assert.equal(
                        ...
                        'The selected class matches the sub class'
                    );
    ...
    QUnit.test('remove a node', function(assert) {
        ...
        resourceSelectorFactory($container, config)
            .on('update.foo', function() {
                ...
                selection = this.getSelection();
                assert.equal(
                    typeof selection[nodeUri],
                    'undefined',
                    'The selection does not contains the removed node'
                );
    ...
    QUnit.test('add a node', function(assert) {
        ...
        resourceSelectorFactory($container, config)
            .on('update.foo', function() {
                ...
                selection = this.getSelection();
                assert.equal(
                    typeof selection[newNode.uri],
                    'undefined',
                    'The selection does not contain the future node'
                );
                ...
                assert.equal(
                    ...
                    parentUri,
                    'The node has been inserted under the correct parent'
                );

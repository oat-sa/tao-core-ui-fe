define(['jquery', 'ui/feedback'], function($, feedback) {
    ...
    QUnit.test('wrong container', function(assert) {
        ...
        assert.throws(
            ...
            TypeError,
            'An exception should be thrown if the container is not an existing element'
        );
    });
            ...
            .on('render', function() {
                ...
                assert.equal(
                    $element
                        ...
                        .trim(),
                    'foo',
                    'The component has the correct message'
                );
                ...
                .on('render', function() {
                    ...
                    assert.equal(
                        $($element)
                            ...
                            .html()
                            .trim(),
                        data.expected.message.trim(),
                        'The component has the correct message'
                    );
                    assert.equal(
                        $element.hasClass('popup'),
                        data.expected.popup,
                        'The component has the correct popup state'
                    );
    ...
    QUnit.test('only one', function(assert) {
        ...
        fb1 = feedback($container).on('render', function() {
        ...
        fb2 = feedback($container).on('render', function() {
            assert.equal($('.feedback', $container).length, 1, 'The contaier has no feedback');

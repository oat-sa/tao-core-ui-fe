define(['lodash', 'ui/themes'], function (_, themesHandler) {
    ...
    QUnit.test('get(what)', function (assert) {
        ...
        assert.deepEqual(
            themesHandler.get('items'),
            config[itemsNs1],
            'automatically returns namespaced entry if an active namespace exists'
        );
        ...
        assert.deepEqual(
            themesHandler.get('items'),
            config.items,
            'returns item themes if specified namespace is not found'
        );
    });
    ...
    QUnit.test('getAvailable(what)', function (assert) {
        ...
        assert.deepEqual(
            themesHandler.getAvailable('items_ns1'),
            config[itemsNs1].available,
            'returns available items_ns1 themes'
        );
        ...
        themesHandler.setActiveNamespace('ns1');
        assert.deepEqual(
            themesHandler.getAvailable('items'),
            config[itemsNs1].available,
            'returns namespaced entry if an active namespace exists'
        );
        ...
        assert.deepEqual(
            ...
            'returns items entry if active namespace is not found'
        );
    });
    ...
    QUnit.test('getAvailable(what, ns)', function (assert) {
        ...
        themesHandler.setActiveNamespace('ns2');
        assert.deepEqual(
            themesHandler.getAvailable('items', 'ns1'),
            config[itemsNs1].available,
            'returns available items themes of entry "items_ns1"'
        );
        assert.deepEqual(
            themesHandler.getAvailable('items', 'unknown'),
            [],
            'returns empty array if namespace is not found'
        );
    });
    ...
    QUnit.test('setActiveNamespace(ns)', function (assert) {
        ...
        assert.deepEqual(
            themesHandler.getActiveNamespace(),
            itemsNs1,
            'Returns items_ns1 as it was just set through setActiveNamespace call'
        );
    });

...
define(['jquery', 'lodash', 'ui/report'], function($, _, report) {
    ...
    QUnit.test('init', function(assert) {
        ...
        assert.equal(
            ...
            config.labelName,
            'The report instance must catch the labelName config'
        );
        assert.equal(
            instance.config.labelText,
            config.labelText,
            'The report instance must catch the labelText config'
        );
        ...
        assert.equal(
            ...
            config.textNumber,
            'The report instance must catch the textNumber config'
        );
        assert.equal(
        ...
        assert.equal(
            ...
            config.selectable,
            'The report instance must catch the selectable config'
        );
    ...
    QUnit.test('simple report', function(assert) {
        ...
        report({}, sampleData)
            .on('render', function() {
                ...
                assert.ok(
                    $component.find('.content > .leaf').hasClass('feedback-warning'),
                    'the report component the type warning'
                );
    ...
    QUnit.test('simple report with actions', function(assert) {
        ...
        report(
            ...
            .on('render', function() {
                ...
                assert.equal(
                    $component.find('.actions .action[data-trigger="rollback"]').length,
                    1,
                    'the report has one rollback button'
                );
                assert.equal(
                    $component.find('.actions .action[data-trigger="continue"]').length,
                    1,
                    'the report has one continue button'
                );
    ...
    QUnit.test('hierarchical report', function(assert) {
        ...
        report({}, sampleData)
            .on('render', function() {
                ...
                assert.ok(
                    $component.find('.content > .hierarchical').hasClass('feedback-warning'),
                    1,
                    'the report component has the type warning'
                );
    ...
    QUnit.test('toggle details', function(assert) {
        ...
        report({}, sampleData)
            ...
            .on('showDetails', function() {
                ...
                //Check hierarchical report visibility
                assert.equal(
                    $component.find('.hierarchical:visible').length,
                    3,
                    'all hierarchical reportr are visible'
                );

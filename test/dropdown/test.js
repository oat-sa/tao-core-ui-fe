...
define(['jquery', 'ui/dropdown'], function ($, dropdown) {
    ...
    QUnit.test('creation', function (assert) {
        ...
        assert.equal(
            dd.getElement().find('.dropdown-header').children().length,
            0,
            'The dropdown header has no children'
        );
        assert.equal(
            dd.getElement().find('.dropdown-submenu').children().length,
            0,
            'The dropdown list has no children'
        );
    ...
    QUnit.test('adding html items', function (assert) {
        ...
        assert.equal(
            dd4.getElement().find('.dropdown-submenu li:last-child span').text(),
            'HTML Item',
            'The dropdown list item was set'
        );

        dd4.setItems(items);
        assert.equal(
            dd4.getElement().find('.dropdown-submenu li').length,
            items.length,
            'The dropdown list was populated with the correct number of items'
        );

...
define(['jquery', 'ui/dashboard'], function($, dashboard) {
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.notStrictEqual(
            dashboard(),
            dashboard(),
            'The dashboard factory provides a different object on each call'
        );
    });
    ...
    QUnit.test('map score to check status', function(assert) {
        ...
        assert.equal(
            instanceWithCustomScoreState.mapScoreToState(scoreState.error),
            'error',
            'The metric score mapped to error status'
        );
        assert.equal(
            instanceWithCustomScoreState.mapScoreToState(scoreState.warn),
            'warn',
            'The metric score mapped to error status'
        );
        assert.equal(
            instanceWithCustomScoreState.mapScoreToState(scoreState.warn + 1),
            'success',
            'The metric score mapped to error status'
        );

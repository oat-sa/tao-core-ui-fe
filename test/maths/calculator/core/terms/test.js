...
define(['lodash', 'ui/maths/calculator/core/terms'], function (_, registeredTerms) {
    ...
    var termTypes = ['digit', 'aggregator', 'separator', 'operator', 'variable', 'constant', 'error', 'function'];
    ...
    QUnit.test('terms', function (assert) {
        ...
        _.forEach(registeredTerms, function (term, id) {
            ...
            assert.ok(
                'string' === typeof term.exponent || null === term.exponent,
                `The property exponent of the term ${id} is null or string`
            );
        });
    });
});

define(['jquery', 'ui/deleter'], function ($, deleter) {
    ...
    QUnit.test('Initialization', function (assert) {
        ...
        const $elt = $('.deleter', $container);
        assert.ok($elt.length === 1, 'Deleter link is available');

        const $target = $('.content', $container);
    ...
    QUnit.test('Undo Delete', function (assert) {
        ...
        $elt.on('undo.deleter', function () {
            const $eventTarget = $('.content', $container);
            assert.ok($eventTarget.length === 1, 'Target is available');
            ready();
        });
        $elt.deleter({
            target: $target,

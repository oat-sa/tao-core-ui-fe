...
define([
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.notStrictEqual(
            classSelector(),
            classSelector(),
            'The classSelector factory provides a different object on each call'
        );
    });
    ...
    QUnit.test('Rendering', function(assert) {
        ...
        classSelector($container, {
            ...
            assert.equal(
                $('.class-selector > a.selected', $container).length,
                1,
                'The selection element has been inserted'
            );
            assert.equal(
                $('.class-selector > a.selected', $container).text(),
                'Select a class',
                'The selection placeholder is correct'
            );
            ...
            assert.equal(
                $('ul:first-child > li:first-child > a', $element).length,
                1,
                'The root class node has been inserted'
            );
            assert.equal(
                $('ul:first-child > li:first-child > a', $element).data('uri'),
                'http://www.tao.lu/Ontologies/TAOSubject.rdf#Subject',
                'The root class uri is correct'
            );
            assert.equal(
                $('ul:first-child > li:first-child > ul > li:last-child > a', $element).data('uri'),
                'http://bertao/tao.rdf#i14727380063820347',
                'The last 1st level node class uri is correct'
            );
    ...
    QUnit.test('selection', function(assert) {
        ...
        classSelector($container, {
            ...
            assert.equal(
                $('a.selected', $element).text(),
                'Trainee',
                "The nod's text is used by the selection element"
            );
    ...
    QUnit.test('empty the component', function(assert) {
        ...
        classSelector($container, {
            ...
            .on('render', function() {
                ...
                assert.equal(
                    this.getValue(),
                    'http://bertao/tao.rdf#i14727195563004295',
                    'There default value is correct'
                );
        ...
        classSelector($container, {
            ...
            assert.equal(
                $('a.selected', $container)
                    ...
                    .trim(),
                label,
                'The node is also the selected node'
            );
            ...
            assert.equal(
                $('a.selected', $container)
                    ...
                    .trim(),
                newLabel,
                'The selection label has also changed'
            );

...
define(['jquery', 'lodash', 'ui/dialog'], function ($, _, dialog) {
    ...
    QUnit.test('install', function (assert) {
        ...
        modal.on('create.modal', function () {
            // This should occur only once
            assert.ok(
                modal.getDom().parent().is(renderTo),
                'When rendered, the dialog box is rendered into target element'
            );
            resolve();
        });
    ...
    QUnit.test('buttons', function (assert) {
        ...
        assert.equal(
            modal.getDom().find('button[data-control="yes"]').length,
            1,
            "The dialog box displays a 'yes' button"
        );
        assert.equal(
            modal.getDom().find('button[data-control="no"]').length,
            1,
            "The dialog box displays a 'no' button"
        );
        assert.equal(
            modal.getDom().find('button[data-control="ok"]').length,
            1,
            "The dialog box displays a 'ok' button"
        );
        assert.equal(
            modal.getDom().find('button[data-control="cancel"]').length,
            1,
            "The dialog box displays a 'cancel' button"
        );
        ...
        assert.equal(
            modal.getDom().find('button[data-control="test"]').length,
            1,
            "The dialog box displays a 'test' button"
        );
        assert.equal(
            modal.getDom().find('button[data-control="test"]').text().trim(),
            'test',
            "The dialog box displays has a 'test' label"
        );
        assert.ok(
            modal.getDom().find('button[data-control="test"]').hasClass('btn-info'),
            "The 'test' button has the 'info' class"
        );
        assert.ok(
            modal.getDom().find('button[data-control="test"]').hasClass('test'),
            "The 'test' button has the 'test' class"
        );
        ...
        assert.equal(
            modal.getDom().find('button[data-control="done"]').length,
            1,
            "The dialog box displays a 'done' button"
        );
        assert.equal(
            modal.getDom().find('button[data-control="done"]').text().trim(),
            'done',
            "The dialog box displays has a 'done' label"
        );
        assert.ok(
            modal.getDom().find('button[data-control="done"]').hasClass('btn-info'),
            "The 'done' button has the 'info' class"
        );
        assert.ok(
            modal.getDom().find('button[data-control="done"]').hasClass('done'),
            "The 'done' button has the 'done' class"
        );
        ...
        .test('focus', function (data, assert) {
            ...
            modal
                .on('create.dialog', function () {
                    ...
                    assert.equal(modal.getDom().get(0), document.activeElement, 'The modal body should be focused now');

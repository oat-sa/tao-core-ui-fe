...
define(['jquery', 'lodash', 'ui/container'], function($, _, container) {
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.notStrictEqual(
            container(),
            container(),
            'The container factory provides a different object on each call'
        );
    });

    QUnit.cases.init(containerApi).test('instance API ', function(data, assert) {
    ...
    QUnit.test('find', function(assert) {
        ...
        assert.equal(
            instance.find('.child').get(0),
            $('#qunit-fixture .child').get(0),
            'The right element has been caught'
        );
    ...
    QUnit.test('write', function(assert) {
        ...
        assert.equal(
            instance
                ...
                .trim(),
            '<div>foo</div>',
            'The container contains the right content'
        );

...
define(['jquery', 'lodash', 'ui/taskQueue/status', 'css!taoCss/tao-main-style.css', 'css!taoCss/tao-3.css'], function(
    ...
    QUnit.test('factory', function(assert) {
        ...
        assert.throws(
            ...
            TypeError,
            'The component needs a not empty context'
        );
        ...
        status
            ...
            .on('running', function() {
                ...
                assert.equal(
                    ...
                    1,
                    'a report has been attached to the status'
                );
                assert.equal(
                    status.$component.find('.component-report .icon-info').length,
                    1,
                    'the status report is of an info type'
                );
                assert.equal(
                    status.$component.find('.component-report .hierarchical').length,
                    0,
                    'the status report has no children'
                );
                assert.equal(
                    status.$component.find('.component-report .fold').length,
                    0,
                    'the status report has no show details button'
                );
        ...
        status
            ...
            .on('finished', function() {
                ...
                assert.equal(
                    status.$component.find('.component-report .fold').length,
                    1,
                    'the status report has the show details button'
                );
                assert.ok(
                    status.$component.find('.component-report .content > .hierarchical').hasClass('feedback-warning'),
                    1,
                    'the status report inherit the type of the report'
                );
                assert.equal(
                    status.$component.find('.component-report .hierarchical').length,
                    4,
                    'the report has 4 hierarchical reports'
                );
                assert.equal(
                    status.$component.find('.component-report .leaf').length,
                    3,
                    'the report has 3 leaf reports'
                );
        ...
        status
            ...
            .on('finished', function() {
                ...
                assert.equal(
                    ...
                    4,
                    'all hierarchical reports are visible'
                );
                assert.equal(
                    status.$component.find('.component-report .leaf:visible').length,
                    3,
                    'all leaf reports are visible'
                );
            ...
            .on('hideDetails', function() {
                assert.equal(
                    status.$component.find('.component-report .hierarchical:visible').length,
                    1,
                    '1 hierarchical report is visible'
                );
                assert.equal(
                    status.$component.find('.component-report .leaf:visible').length,
                    0,
                    'no leaf report is visible'
                );

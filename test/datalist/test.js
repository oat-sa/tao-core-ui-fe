 ...
 */
define(['jquery', 'lodash', 'ui/datalist'], function ($, _, datalist) {
    'use strict';
    ...
    QUnit.test('init', function (assert) {
        ...
        assert.equal(
            ...
            false,
            'The datalist instance must not accept undefined config properties'
        );
        assert.equal(
            Object.prototype.hasOwnProperty.call(instance, 'dummy'),
            false,
            'The datalist instance must not accept null config properties'
        );
        assert.equal(instance.config.keyName, config.keyName, 'The datalist instance must catch the keyName config');
        assert.equal(
            instance.config.labelName,
            config.labelName,
            'The datalist instance must catch the labelName config'
        );
        assert.equal(
            instance.config.labelText,
            config.labelText,
            'The datalist instance must catch the labelText config'
        );
        assert.equal(instance.config.title, config.title, 'The datalist instance must catch the title config');
        assert.equal(
            instance.config.textNumber,
            config.textNumber,
            'The datalist instance must catch the textNumber config'
        );
        assert.equal(
            instance.config.textEmpty,
            config.textEmpty,
            'The datalist instance must catch the textNumber config'
        );
        assert.equal(
            instance.config.textLoading,
            config.textLoading,
            'The datalist instance must catch the textNumber config'
        );
        assert.equal(
            ...
            config.selectable,
            'The datalist instance must catch the selectable config'
        );
    ...
    QUnit.cases.init(datalistConfigs).test('render ', function (data, assert) {
        ...
        assert.equal(
            $container.find('.dummy').length,
            0,
            'The container does not contain an element of the class dummy'
        );
        assert.equal(instance.is('rendered'), true, 'The datalist instance must be rendered');
        assert.equal(
            typeof instance.getElement(),
            'object',
            'The datalist instance returns the rendered content as an object'
        );
        ...
        assert.equal(
            ...
            $container.get(0),
            'The datalist instance is rendered inside the right container'
        );

        assert.equal(
            instance.getElement().find('h1').text(),
            config.title,
            'The datalist instance has rendered a title with the right content'
        );
        assert.equal(
            instance.getElement().find('.empty-list').text(),
            config.textEmpty,
            'The datalist instance has rendered a message to display when the list is empty, and set the right content'
        );
        assert.equal(
            instance.getElement().find('.available-list .label').text(),
            config.textNumber,
            'The datalist instance has rendered a message to show the number of boxes, and set the right content'
        );
        assert.equal(
            instance.getElement().find('.available-list .count').text(),
            datalistData.length,
            'The datalist instance displays the right number of lines'
        );
        assert.equal(
        ...
        if (config.tools) {
            assert.equal(
                instance.getElement().find('.list .action-bar').length,
                1,
                'The datalist instance has rendered an action bar'
            );

            assert.equal(
                instance.getElement().find('.list .action-bar button').length,
                config.tools.length,
                'The datalist instance has rendered buttons in the action bar'
            );
            _.forEach(config.tools, function (tool) {
                ...
                assert.equal(
                    instance.getElement().find(`.list .action-bar [data-control="${tool.id}"]`).text().trim(),
                    tool.label,
                ...
                if (tool.massAction) {
                    ...
                    assert.equal(
                        instance
                            ...
                            .hasClass('mass-action'),
                        false,
                        `The datalist instance has rendered the tool button ${tool.id} without the class mass-action`
                    );
                }
            });
        } else {
            assert.equal(
                instance.getElement().find('.list .action-bar').length,
                0,
                'The datalist instance must not render an action bar'
            );
        }
        ...
        assert.equal(
            instance.getElement().find('.list th.label').length,
            1,
            'The datalist instance has rendered a label header'
        );
        assert.equal(
            instance.getElement().find('.list th.label').text().trim(),
            config.labelText,
            'The datalist instance has rendered the right text in the label header'
        );
        if (config.selectable) {
            assert.equal(
                instance.getElement().find('.list th.checkboxes input').length,
                1,
                'The datalist instance has rendered a checkbox header'
            );
        } else {
            assert.equal(
                instance.getElement().find('.list th.checkboxes input').length,
                0,
                'The datalist instance must not render a checkbox header'
            );
        }

        assert.equal(
            instance.getElement().find('.list tbody tr').length,
            datalistData.length,
            'The datalist instance has rendered the right number of lines'
        );
                ...
                _.forEach(config.actions, function (action) {
                    if (!action.hidden || !action.hidden.call(line)) {
                        assert.equal(
                            ...
                            1,
                            `The datalist instance has rendered the action button ${action.id} in the line with id ${id}`
                        );
                        assert.equal(
                            $line.find(`td.actions [data-control="${action.id}"]`).text().trim(),
                            action.label,
                            `The datalist instance has rendered the action button ${action.id} with the label ${action.label} in the line with id ${id}`
                        );
                        ...
                        if (action.icon) {
                            assert.equal(
                                ...
                                1,
                                `The datalist instance has rendered the action button ${action.id} with an icon in the line with id ${id}`
                            );
                            assert.equal(
                                $line
                                ...
                                true,
                                `The datalist instance has rendered the action button ${action.id} with the icon ${action.icon} in the line with id ${id}`
                            );
                        } else {
                            assert.equal(
                                $line.find(`td.actions [data-control="${action.id}"] .icon`).length,
                                0,
                                `The datalist instance has rendered the action button ${action.id} without an icon in the line with id ${id}`
                            );
                        }
                    } else {
                        assert.equal(
                            $line.find(`td.actions [data-control="${action.id}"]`).length,
                            0,
                            `The datalist instance must not render the hidden action button ${action.id} in the line with id ${id}`
                        );
                    }
                });
            } else {
    ...
    QUnit.cases.init(datalistConfigs).test('update ', function (data, assert) {
        var $dummy = $('<div class="dummy" />');
        var $container = $('#fixture-1').append($dummy);
        var datalistData = [
            {
    ...
    QUnit.test('show/hide', function (assert) {
        ...
        assert.equal(
            ...
            false,
            'The datalist instance does not have the hidden class'
        );
    ...
    QUnit.test('enable/disable', function (assert) {
        ...
        assert.equal(
            ...
            false,
            'The datalist instance does not have the disabled class'
        );
    ...
    QUnit.test('state', function (assert) {
        ...
        assert.equal(
            ...
            false,
            'The datalist instance does not have the customState class'
        );
        ...
        assert.equal(
            instance.getElement().hasClass('customState'),
            true,
            'The datalist instance has the customState class'
        );
    ...
    QUnit.test('events', function (assert) {
        ...
        const ready5 = assert.async();
        const ready4 = assert.async();
        ...
        const instance = datalist(config);
        ...
        instance.on('tool', function (selection, buttonId) {
            ...
            assert.ok(
                _.isArray(selection),
                'The datalist instance provides the current selection when a tool button is clicked'
            );
            assert.equal(
                buttonId,
                'tool1',
                'The datalist instance provides the button identifier when a tool button is clicked'
            );
            ready3();
        ...
        instance.on('action', function (lineId, buttonId) {
            ...
            assert.equal(
                lineId,
                '1',
                'The datalist instance provides the line identifier when an action button is clicked'
            );
            assert.equal(
                buttonId,
                'action1',
                'The datalist instance provides the button identifier when an action button is clicked'
            );
            ready4();

            instance
                .getElement()
                .find('[data-id="1"] td.checkboxes input')
                .on('click', function () {
        ...
        instance.on('select', function (selection) {
            ...
            assert.ok(
                _.isArray(selection),
                'The datalist instance provides the current selection when the select event is triggered'
            );
            assert.equal(
                selection.length,
                1,
                'The datalist instance provides the right selection when the select event is triggered'
            );
            assert.equal(
                selection[0],
                '1',
                'The datalist instance provides the right lines identifiers when the select event is triggered'
            );
            ready5();
        });
        ...
        instance.render().update(data).trigger('custom').destroy();
    });
    ...
    QUnit.test('selection', function (assert) {
        ...
        instance.on('render', function () {
            assert.equal(
                instance.getElement().find('[data-control="tool1"]').hasClass('mass-action'),
                false,
                'The tool1 button is not a mass action'
            );
            assert.equal(
                instance.getElement().find('[data-control="tool1"]').hasClass('hidden'),
                false,
                'The tool1 button is visible'
            );
            assert.equal(
                instance.getElement().find('[data-control="tool2"]').hasClass('mass-action'),
                true,
                'The tool2 button is a mass action'
            );
            assert.equal(
                instance.getElement().find('[data-control="tool2"]').hasClass('hidden'),
                true,
                'The tool2 button is hidden'
            );
        ...
        instance.on('select', function (sel) {
            assert.equal(
                instance.getElement().find('[data-control="tool2"]').hasClass('hidden'),
                !(sel && sel.length),
                'The tool2 button must be visible if a selection is made'
            );
        });
        ...
        instance.on('update', function () {
            // Check pre-selection
            assert.equal(
                instance.getElement().find('tbody tr').length,
                data.length,
                'The datalist instance has rendered the exact number of lines'
            );
            assert.equal(
                instance.getElement().find('tbody .checkboxes').length,
                data.length,
                'The datalist instance has rendered the checkboxes'
            );

            assert.equal(
                instance.getElement().find('tbody input:checked').length,
                selection.length,
                'The datalist instance has made a selection'
            );
            ...
            assert.equal(
                instance.getElement().find('tbody input:checked').length,
                selection.length,
                'The datalist instance has updated the selection'
            );
            ...
            assert.equal(
                instance.getElement().find('tbody input:checked').length,
                0,
                'The datalist instance has cleared the selection'
            );

...
define([
    ...
    QUnit.test('highlight all', function(assert) {
        ...
        assert.ok(
            ...
            'The highlightAll option is not checked'
        );
    ...
    QUnit.test('search case sensitive', function(assert) {
        ...
        assert.equal($('.pdf-find-bar', $container).length, 1, 'The find bar has been added');
        assert.ok(
            !$('[data-control="case-sensitive-search"]', $container).is(':checked'),
            'The caseSensitive option is not checked'
        );
            ...
            .then(function() {
                ...
                events
                    ...
                    .on('searchdone', function(query, page) {
                        ...
                        assert.deepEqual(
                            instance.getSearchEngine().getMatches(),
                            expectedMatches,
                            'The search has found the expected matches'
                        );
                    })
                    ...
                    .on('matchesupdated', function(page) {
                        ...
                        assert.equal(
                            !$('[data-control="pdf-search-loop-begin"]', $container).hasClass('hidden'),
                            current.loopBegin,
                            'The loop to begin message is correctly set'
                        );
                        assert.equal(
                            !$('[data-control="pdf-search-loop-end"]', $container).hasClass('hidden'),
                            current.loopEnd,
                        ...
                        assert.equal(
                            instance.getSearchEngine().getCurrentMatch().page,
                            current.page,
                            'The right page is set'
                        );
                        assert.equal(
                            instance.getSearchEngine().getCurrentMatch().index,
                            current.index,
                            'The right page match is selected'
                        );
                        assert.equal(
                            instance.getSearchEngine().getCurrentMatch().overall,
                            current.overall,
                            'The right match is selected'
                        );
    ...
    QUnit.test('destroy', function(assert) {
        ...
        assert.ok(
            _.isPlainObject(instance.getSearchEngine()),
            'The getSearchEngine() method returns the search engine instance'
        );
        assert.equal($('[data-control="pdf-search"]', $container).length, 1, 'The search button has been added');
        assert.equal($('.pdf-find-bar', $container).length, 1, 'The find bar has been added');

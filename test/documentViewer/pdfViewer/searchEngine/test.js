            ...
            .then(function(pdf) {
                ...
                return instance.search(data.query, data.currentPage).then(function(pageNum) {
                    assert.equal(
                        pageNum,
                        data.firstPage,
                        'The search has found the terms and returned the right page number'
                    );
                    assert.equal(
                        instance.getMatches().length,
                        pdfjs.pageCount,
                        'The matches collection contains the same numbers than the amount of pages'
                    );
                    assert.deepEqual(instance.getMatches(), data.matches, 'The search has find the expected matches');
                    assert.deepEqual(
                        instance.getPages(),
                        data.pageNumbers,
                        'The search has find matches in the expected pages'
                    );
                    ...
                    assert.equal(
                        instance.getCurrentMatch().page,
                        data.firstPage,
                        'The current match target the right page'
                    );
                ...
                return instance.search(query, currentPage).then(function(pageNum) {
                    ...
                    _.forEach(matchesPathPrevious, function(expectedMatch) {
                        ...
                        assert.equal(
                            loop,
                            expectedMatch.loop,
                            'We can navigate seamlessly across match all over the document'
                        );
                        ...
                        assert.equal(
                            match.overall,
                            expectedMatch.overall,
                            'The current match target the right overall index'
                        );
                    });
                                ...
                                return textManager.renderPage(pageNum, page.getViewport()).then(function(layer) {
                                    return instance.updateMatches(pageNum).then(function(num) {
                                        ...
                                        assert.equal(
                                            $matches.length,
                                            6,
                                            'There is the right number of highlighted matches'
                                        );
                                        ...
                                        assert.equal(
                                            $matches.eq(0).text(),
                                            'pa',
                                            'The highlighted span contains the right text'
                                        );
                                        assert.equal(
                                            $matches.eq(0).data('match'),
                                            '0',
                                            'The highlighted span is related to the right match'
                                        );

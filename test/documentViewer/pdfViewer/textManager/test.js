...
define([
    ...
    QUnit.test('setDocument', function (assert) {
        ...
        assert.equal(
            ...
            null,
            'The getDocument() method returns null when no document have been set'
        );
                        ...
                        return instance.getContents().then(function (contents) {
                            ...
                            _.forEach(contents, function (pageContent, index) {
                                ...
                                assert.equal(
                                    ...
                                    'object',
                                    'The page content contains a content entry'
                                );
                                assert.equal(
                                    typeof pageContent.strings,
                                    'object',
                                    'The page content contains a strings entry'
                                );
                                assert.equal(
                                    typeof pageContent.text,
                                    'string',
                                    'The page content contains a text entry'
                                );
                                assert.ok(
                                    pageContent.nodes instanceof Array,
                                    'The page content contains a nodes entry'
                                );

                                assert.deepEqual(
                                    pageContent.strings,
                                    textContent[index],
                                    'The page content contains the right strings'
                                );
                                assert.equal(
                                    pageContent.text,
                                    textContent[index].join(''),
                            ...
                            return instance.getContents().then(function (_contents) {
                                assert.equal(
                                    contents,
                                    _contents,
                                    'The getContents() method always returns the same object unless the document has been changed'
                                );
                    ...
                    .then(function (pdf) {
                        ...
                        return instance.getPageContent(2).then(function (pageContent2) {
                            ...
                            return instance.getPageContent(1).then(function (pageContent1) {
                                ...
                                assert.equal(
                                    typeof pageContent1.strings,
                                    'object',
                                    'The page content contains a strings entry'
                                );
                                assert.equal(
                                    typeof pageContent1.text,
                                    'string',
                                    'The page content contains a text entry'
                                );
                                assert.ok(
                                    pageContent1.nodes instanceof Array,
                                    'The page content contains a nodes entry'
                                );

                                assert.deepEqual(
                                    pageContent1.strings,
                                    textContent[0],
                                    'The page content contains the right strings'
                                );
                                assert.equal(
                ...
                return pdf.getPage(1).then(function (page) {
                    return Promise.all([
                        ...
                        assert.equal(
                            typeof results[1],
                            'object',
                            'The result of the second rendering process is an object'
                        );

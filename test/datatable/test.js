...
define([
    ...
    QUnit.test('Initialization', function (assert) {
        ...
        const firstUrl = '/test/datatable/data.json';
        const secondUrl = '/test/datatable/largedata.json';
    ...
    QUnit.test('Options', function (assert) {
        ...
        $elt.on('create.datatable', function () {
            ...
            let data = $elt.data('ui.datatable') || {};
    ...
    QUnit.test('Option readonly', function (assert) {
        ...
        $container.on('create.datatable', function () {
            assert.equal(
                $('[data-item-identifier="1"] button.disabled', $container).length,
                3,
                'All action buttons are disabled for readonly="true"'
            );
            assert.ok(
                $('[data-item-identifier="2"] button.disabled', $container).length === 1 &&
                    $('[data-item-identifier="2"] button.view', $container).hasClass('disabled'),
                'Specific action button is disabled for readonly={ 2: { view: true}}'
            );
            assert.equal(
                $('[data-item-identifier="3"] button.disabled', $container).length,
                0,
    ...
    QUnit.test('Option requestInterceptor failed', function (assert) {
        ...
        $container.on('error.datatable', function () {
    ...
    QUnit.test('Model loading using predefined data', function (assert) {
        ...
        $elt.on('query.datatable', function () {
        ...
        $elt.one('load.datatable', function (event, response) {
            ...
            _.defer(function () {
                $elt.one('load.datatable', function (loadEvent, loadResponse) {
                    assert.equal(
                        typeof loadResponse,
                        'object',
    ...
    QUnit.test('Model loading with the "action" type property using predefined data', function (assert) {
        ...
        $elt.on('beforeload.datatable', function (response) {
            assert.equal(typeof response, 'object', 'the beforeload event is triggered and provides the response data');
            ready1();
        });
        $elt.one('load.datatable', function (response) {
            assert.equal(typeof response, 'object', 'the load event is triggered and provides the response data');
            assert.equal(
                $elt.find('.datatable tbody tr').length,
                dataset.data.length,
    ...
    QUnit.test('Model loading with actions column', function (assert) {
        ...
        $elt.on('create.datatable', function () {
            ...
            assert.equal(
                $elt.find('.datatable thead th:eq(6) > div').html(),
                '',
                'Column type "action" without actions'
            );
    ...
    QUnit.test('Selection with massAction buttons', assert => {
        ...
        $container
            .on('create.datatable', function () {
                ...
                assert.notOk(
                    $massActionButton.hasClass('invisible'),
                    'Mass action button is visible once one row is selected'
                );

                $selectAll.trigger('click');
                assert.notOk(
                    $massActionButton.hasClass('invisible'),
                    'Mass action button is visible when all rows are selected'
                );

                $selectAll.trigger('click');
                assert.ok(
                    $massActionButton.hasClass('invisible'),
                    'Mass action button is invisible when all rows are unselected'
                );
    ...
    QUnit.test('Default filtering enabled', function (assert) {
        ...
        $elt.on('create.datatable', function () {
            ...
            const $filterInput = $elt.find('.datatable-wrapper aside.filter input');
            const eventKeyPressH = $.Event('keypress', { which: 72 });
            const eventKeyPressEnter = $.Event('keypress', { which: 13 });
    ...
    QUnit.test('Transform', function (assert) {
        ...
        const $elt = $('#container-1');
        const renderFullName = function (row) {
        ...
        const transform = function (value, row, field, index, data) {
            ...
            assert.equal(data, datasetMock, 'The provided dataset is the right dataset');

            assert.equal(row, datasetMock[index], 'The provided row is the exact row at index');
        ...
        $elt.on('create.datatable', function () {
            ...
            assert.equal(
                $elt.find('.datatable thead th:eq(0)').text().trim(),
                model[0].label,
                'The first column contains the right header'
            );
            assert.equal(
                $elt.find('.datatable thead th:eq(1)').text().trim(),
                model[1].label,
            ...
            assert.equal(
                $elt.find('.datatable tbody tr:eq(0) td:eq(0)').text().trim(),
                renderFullName(datasetMock[0]),
                'The first line contains the right full name'
            );
            assert.equal(
                $elt.find('.datatable tbody tr:eq(0) td:eq(1)').text().trim(),
                datasetMock[0].email,
                'The first line contains the right email'
            );

            assert.equal(
                $elt.find('.datatable tbody tr:eq(1) td:eq(0)').text().trim(),
                renderFullName(datasetMock[1]),
                'The second line contains the right full name'
            );
            assert.equal(
                $elt.find('.datatable tbody tr:eq(1) td:eq(1)').text().trim(),
                datasetMock[1].email,
    ...
    QUnit.cases
        ...
        .test('Pagination strategy', function (strategy, assert) {
            ...
            $container.one('create.datatable', () => {
                assert.equal(
                    $('.datatable-pagination-top').children().length,
                    paginationTopChildrenLength,
                    'Top pagination content is rendered'
                );
                assert.equal(
                    $('.datatable-pagination-bottom').children().length,
                    paginationBottomChildrenLength,
                    'Bottom pagination content is rendered'
                );
                done();
            });
    ...
    QUnit.test('Pagination', function (assert) {
        ...
        $container.on('create.datatable', function () {
            ...
            $container.one('setpage.datatable', function () {
                assert.ok(
                    $('.pages .page', $container).eq(2).hasClass('active'),
                    'The third page is active after click on pagination button'
                );
            });

            $container.one('backward.datatable', function () {
                assert.ok(
                    $('.pages .page', $container).eq(1).hasClass('active'),
                    'The first page is active after click on next button'
                );
            });
            ...
            $container.one('forward.datatable', function () {
                assert.ok(
                    $('.pages .page', $container).eq(2).hasClass('active'),
                    'The second page is active after click on previous button'
                );
                done();
            });
    ...
    QUnit.test('Endless listeners on events', function (assert) {
        ...
        $elt.on('create.datatable', function () {
            ...
            const eventPressKey = $.Event('keyup', { keyCode: 72 });
        ...
        .test('Sortable headers', function (data, assert) {
            ...
            $container
                ...
                .on('sort.datatable', function (e, sortby, sortorder, sorttype) {
                    ...
                    assert.notEqual(sortorder, void 0, 'The sort order passed via event');
    ...
    QUnit.test('Columns with alias', function (assert) {
        ...
        $container
            .on('create.datatable', function () {
                ...
                assert.equal(
                    $('.datatable thead [data-sort-by="login"] .alias', $container).length,
                    1,
                    'The column alias is rendered'
                );
                assert.equal(
                    $('.datatable thead [data-sort-by="email"] .alias', $container).length,
                    0,
                    'Column without alias does not show alias'
                );
    ...
    QUnit.test('Columns with comment', function (assert) {
        ...
        $container
            .on('create.datatable', function () {
                ...
                assert.equal(
                    $('.datatable thead [data-sort-by="login"] .comment', $container).length,
                    1,
                    'The column comment is rendered'
                );
                assert.equal(
                    $('.datatable thead [data-sort-by="email"] .comment', $container).length,
                    0,
                    'Column without comment does not show comment'
                );
        ...
        .test('Atomic update', function (param, assert) {
            ...
            $container.one('create.datatable', function () {
                ...
                $container.one('load.datatable', function () {
                    ...
                    assert.equal(
                        statusColumn1.get(0),
                        $container.find('.status').get(0),
                        'The DOM elements for table cells is not changed'
                    );
                    ...
                    $container.one('load.datatable', function () {
                        ...
                        assert.notEqual(
                            statusColumn1.get(0),
                            firstStatusRow,
                            'Force update should be applied since number of rows is decreased'
                        );
                        ...
                        $container.one('load.datatable', function () {
                            assert.notEqual(
                                firstStatusRow,
                                $container.find('.status').get(0),
                                'Force update should be applied since the order of rows is changed'
                            );
                            ready();
                        });
    ...
    QUnit.test('It does not render the element is removed while loading', function (assert) {
        ...
        const $elt = $('.table-container', $container);
        assert.ok($elt.length === 1, 'Test the fixture is available');

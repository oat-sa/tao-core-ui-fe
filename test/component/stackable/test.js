 ...
 */
define(['jquery', 'ui/hider', 'ui/stacker', 'ui/component', 'ui/component/stackable'], function(
    $,
    hider,
    stackerFactory,
    componentFactory,
    ...
    QUnit.cases.init([{ title: 'bringToFront' }]).test('stackable component API', function(data, assert) {
    ...
    QUnit.test('bring component to front on .bringToFront()', function(assert) {
        ...
        assert.equal(
            $element.get(0).style.zIndex,
            stacker.getCurrent(),
            'component has been brought back on the front'
        );
    });

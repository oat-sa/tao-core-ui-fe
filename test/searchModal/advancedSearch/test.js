    ...
    QUnit.cases
        ...
        .test('advancedSearch criteria manipulation', function (data, assert) {
            ...
            instance.on('criteriaListUpdated', function () {
                ...
                assert.equal(
                    $criteriaContainer.find('.inBothTextParentUri-filter').length,
                    1,
                    'criterion is added to DOM'
                );
                assert.equal(
                    $criteriaSelect.select2('opened'),
                    false,
                    'critera select is closed after adding a criteria'
                );
                ...
                instance.off('criteriaListUpdated').on('criteriaListUpdated', function () {
                    ...
                    assert.equal(
                        $criteriaContainer.find('.inChildTextUri-filter').length,
                        1,
                        'criterion is added to DOM'
                    );
                ...
                instance.off('ready').on('ready', function () {
                    if (data.enabled) {
                        assert.equal(
                            !$addCriteria.hasClass('disabled'),
                            data.enabled,
                            'critera select is enabled because status is enabled'
                        );
                        return;
                    }
                    assert.equal(
                        $addCriteria.hasClass('disabled'),
                        data.enabled,
                        'critera select is disabled because status is disabled'
                    );
                });
            });
        });
    QUnit.test('bind between view and model is correctly set', function (assert) {
        ...
        instance.on('ready', function () {
            ...
            instance.updateCriteria('undefined/tao/ClassMetadata/').then(async function () {
                ...
                assert.deepEqual(
                    $criterionListSelected.select2('val'),
                    ['value1'],
                    'list criterion correctly initialized'
                );
                ...
                assert.equal(
                    instance.getState()['inBothTextParentUri'].value,
                    'foo0',
                    'text criterion correctly updated'
                );
                assert.deepEqual(

...
define(['jquery', 'lodash', 'ui/dynamicComponent'], function($, _, dynamicComponent) {
    ...
    QUnit.test('render', function(assert) {
        ...
        dynamicComponent({}, {})
            .on('rendercontent', function($content) {
                ...
                assert.equal(
                    $container1.find('.dynamic-component-container').length,
                    1,
                    'Dynamic component container ok'
                );
                assert.equal(
                    $container1.find('.dynamic-component-container .dynamic-component-title-bar').length,
                    1,
                    'Dynamic component title ok'
                );
                assert.equal(
                    $container1.find('.dynamic-component-container .dynamic-component-title-bar .closer').length,
                    1,
                    'Dynamic component title has closer'
                );
                assert.equal(
                    $container1.find('.dynamic-component-container .dynamic-component-content').length,
                    1,
                    'Dynamic component has content'
                );
                assert.equal(
                    $container1.find('.dynamic-component-container .dynamic-component-content .my-custom-content')
                        .length,
                    1,
                    'Dynamic component has content'
                );
                assert.equal(
                    $container1
                        .find('.dynamic-component-container .dynamic-component-content .my-custom-content')
                        .text(),
                    'BBB',
                    'content ok'
                );
                assert.equal(
                    $container1.find(
                        '.dynamic-component-container .dynamic-component-resize-container .dynamic-component-resize'
                    ).length,
                    1,
                    'resize control is rendered'
                );
    ...
    QUnit.test('reset', function(assert) {
        ...
        assert.equal(
            $container1.find('.dynamic-component-container .dynamic-component-content .my-custom-content').text(),
            'BBB',
            'Dynamic component has content'
        );
        ...
        instance.on('reset', function() {
            ...
            assert.equal(
                $container1.find('.dynamic-component-container .dynamic-component-content .my-custom-content').text(),
                '',
                'Dynamic component has emptied content'
            );
        ...
        dynamicComponent({}, {})
            .after('rendercontent', function($content) {
                ...
                var diffHeight =
                    $element.outerHeight() -
                    $element.height() +
                    $element.find('.dynamic-component-title-bar').outerHeight();
                ...
                this.on('resize.setContentSize', function() {
                    ...
                    assert.equal(
                        $element.find('.dynamic-component-content .my-custom-content').length,
                        1,
                        'Dynamic component has content'
                    );
                    assert.equal(
                        $element.find('.dynamic-component-content .my-custom-content').text(),
                        '',
                        'Dynamic component has emptied content'
                    );
